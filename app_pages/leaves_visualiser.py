...
def leaves_visualiser_body():
    ...
    if st.checkbox("""Difference between average and variability image"""):
        ...
        average_powdery_mildew_path = os.path.join(
            root_dir, "streamlit_images", "average-image-for-powdery-mildew.png")
        st.write(f"- Powdery Mildew image path: `{average_powdery_mildew_path}`")
        ...
        try:
            average_powdery_mildew_image = plt.imread(average_powdery_mildew_path)
            st.image(
            ...
            st.error("Error: Could not find average powdery mildew image file.")

        st.write("""
            The average images show the general characteristics of healthy and infected leaves.
            The healthy leaf image has a smooth texture, while the infected leaf image shows a 
            rough texture with white stripes.
            This visual difference supports the hypothesis that there is a noticeable visual difference""")

    if st.checkbox("""Differences between average infected leaves and average healthy leaves"""):
        ...
        try:
            ...
            st.image(
                ...
                caption="Average difference between healthy and infected leaves", 
                use_container_width=True
            )
        except FileNotFoundError:
        ...
        st.write("""
            The differences between the average infected leaves and average healthy leaves 
    ...
    def image_montage(dir_path, nrows, ncols, figsize):
        ...
        st.write(f"Found {len(image_files)} image files. Creating a montage...")
        
        # Iterate over a limited number of files for the montage
        for i, (ax, img_file) in enumerate(zip(axes.flatten(), image_files[:nrows*ncols])):
            # Correctly construct the full path to the image file
    ...
    if st.checkbox("""Image montage"""):
        st.info("""To refresh the image montage, please select the 'Create montage' button.""")
        
        # The path for the image montage is now constructed correctly from the root_dir
        my_data_dir = os.path.join(root_dir, 'inputs', 'cherry-leaves', 'cherry-leaves')
        ...
        try:
            ...
            if st.button("Create montage"):
                # FIX: Correcting the path to include the selected label
            ...
            st.error("Error: Could not find the main data directory. Please check the path displayed above.")
    st.write("""---""")